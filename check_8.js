...
// Evita que una excepción de interfaz deje la PWA bloqueada indefinidamente.
const startupWatchdog=setTimeout(()=>{
  if(!gate.classList.contains('hidden') && currentUser){
    console.warn('Watchdog de inicio: liberando interfaz');
    openApp();
    setSync(navigator.onLine?'saving':'offline',navigator.onLine?'Recuperando sincronización…':'Sin conexión · datos locales');
    try{window.renderAll?.()}catch(error){console.error(error)}
    setTimeout(()=>pushState(),900);
  }
},12000);
...
function openApp(){clearTimeout(startupWatchdog);gate.classList.add('hidden');document.body.classList.remove('firebase-locked');syncPill.hidden=false;}
...
async function pushState(){if(!currentUser||!currentHouseholdId)return;try{const payload=sanitizeState(window.NEBridge.getData());await setDoc(stateRef(),{state:payload,updatedAt:serverTimestamp(),updatedBy:currentUser.uid,version:'5.8.1'},{merge:true});setSync(navigator.onLine?'online':'offline',navigator.onLine?'Sincronizado':'Sin conexión · cambios pendientes');}catch(error){console.error(error);setSync('error','Error de sincronización');}}
...
async function connectHousehold(profile){currentHouseholdId=profile.householdId;currentInviteCode=profile.inviteCode||'';window.NEBridge.household=currentHouseholdId;$('accountInviteCode').textContent=currentInviteCode||'------';setStage('loading','Cargando datos compartidos…');
 ...
 unsubscribeState=onSnapshot(stateRef(),{includeMetadataChanges:true},async snap=>{
   ...
   if(snap.exists()&&snap.data()?.state){
     try{remoteApplying=true;window.NEBridge.applyRemote(snap.data().state);}
     catch(renderError){console.error('No se pudo renderizar el estado remoto',renderError);try{window.NEBridge.applyRemote(window.NEBridge.getData())}catch(_){};setTimeout(()=>toast('Se recuperó la sesión. Algunas vistas se actualizarán al abrirlas.'),300);}
     finally{remoteApplying=false;openApp();}
     setSync(pending?'saving':(navigator.onLine?'online':'offline'),pending?'Guardando…':(navigator.onLine?'Sincronizado':'Sin conexión'));
